...
module.exports = function markdownitLineNumber(md, option = {}) {
  ...
  if (option.codeBlock) {
    md.renderer.rules.code_block = function codeBlockRenderer(
      tokens,
      idx,
      options,
      env,
  ...
  if (option.text) {
    md.renderer.rules.text = function(tokens, idx) {
      ...
      const escapeHtml = md.utils.escapeHtml
      if (token.content) {
        return `<span ${lineAttr(option.startLine, token._line)}>${escapeHtml(
          token.content
        )}</span>`
      } else {
        // Ignore whitespace content such as `**a**`
